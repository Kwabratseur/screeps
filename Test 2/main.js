...
*
*   -   improve defense script(double wall) *** NOT DONE, *** single walls are good enough to start with. Change manually
...
*       * ALWAYS repair the weakest link first, so sort from most damaged to least damaged      ***COMPLETED***
...
module.exports.loop = function () {
    ...
    if(energymon.length < Nos[5]) {
        var newName = Game.spawns.Spawn1.createCreep(TransportCreep(0,10), undefined, {role: 'energymon',Home: MyRoom.roomName});
    ...
    if(worker.length < Nos[2]) {
        var newName = Game.spawns.Spawn1.createCreep(WorkCreep(0), undefined, {role: 'worker',Home: MyRoom.roomName});
