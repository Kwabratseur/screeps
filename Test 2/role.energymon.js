    ...
    run: function(creep,No) {
        ...
        if(link != undefined && link.energy > 0 && No == 0){
            if(creep.carry.energy == 0){ // if creep is empty
                ...
                Transfer.to(creep,creep.room.name,"Storages",RESOURCE_ENERGY,0);
                /*if(creep.transfer(storages, RESOURCE_ENERGY) == ERR_NOT_IN_RANGE){
                    //creep.moveTo(storages, {reusePath: 5});
                    Moveto.move(creep,storages);
                }*/
            }
        }else if(_.sum(storages.store) > 0){ //if storage is not empty
            if(creep.carry.energy == 0){ // if creep is empty
                Transfer.from(creep,creep.room.name,"Storages",RESOURCE_ENERGY,0);
