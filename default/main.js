...
Build.CreepDemand = function(MyRoom,RCL,SAlt,Hostiles,sources,buffer,spawn,links,NewCreepDemand){// Function returns energy/cycle and mincarryparts. Also sets room creeps if NewCreepDemand=true.
  ...
  if(NewCreepDemand){
      Memstructures.setroomcreep(MyRoom,farmers,transporters,workers,army,WorkSize,1,0.3,Math.round(army/2))
  }
...
Build.ExtCreepDemand = function(MyRoom){
  ...
  if(Game.rooms[MyRoom.roomName] == undefined && Memory.roomdb[MyRoom.roomName].RoomInfo == undefined){
    ...
    if(Game.rooms[MyRoom.roomName] != undefined){
...
Build.AutoQueue = function(MyRooms){
  ...
  var healDemand = 0;
    ...
    if((Game.rooms[MyRooms] == undefined) || (Game.rooms[MyRooms].controller.owner == undefined) || (Game.rooms[MyRooms].controller.owner.username != 'Kwabratseur')){ //rooms which are not formally owned!
      ...
      var WorkSize = CreepDemand.Farmers[1];
    }
  ...
  if(Memory.Spawning == 0){
      if((farmersPresent[0].length + farmerInQueue) < FarmDemand){
        ...
        Memstructures.AddtoQueue(0.1,Build.Layout(energyCap,energyCap,WorkSize,"Work"),'farmer',dest,to,from,flag,jobMiner);
      }
...
Build.SpawnCreep = function(){
  ...
  if(CreeptoSpawn != undefined){
    ...
    if(newName == -6){
      ...
      if(Memory.failedSpawn > 3 && CreeptoSpawn[1].length > 3 && Build.Cost(body) > 300 && Memory.Spawning < 1){
        console.log('3 consecutive fails in spawning, Bodypart will be removed.');
    ...
    if(newName == -10){//purges corrupt creeps from memory
      ...
      if(CreeptoSpawn != undefined){
        ...
        console.log('Tried to spawn: '+CreeptoSpawn[2]+'. Energy Available:'+availableEnergies[j]+', creepcost:'+Build.Cost(body));
      }
    }else{
...
tower.run = function(MyRoom,towers,ramparts,containers,hostiles,roads){

  if(towers.length > 0 && towers[0] != null){

      var damagedStructures = ramparts;
      damagedStructures = _.filter(damagedStructures, function(structure){return (structure.hits < 300000); });
      //var damagedStructures = damagedStructures.concat(roads,towers,Game.spawns[SpawnName])
      damagedStructures = damagedStructures.concat(containers,roads);
      var c = 0;
      ...
      for(var id in towers){
          ...
          if(damagedStructures[c] && tower.energy > tower.energyCapacity*0.89){
            console.log('Tower Repair target:'+damagedStructures[c].pos.x+','+damagedStructures[c].pos.y+'. HP:'+damagedStructures[c].hits);
...
if( Memory.stats.HostilesInTerritory == 0 && Memory.stats.RegenBucket == 1){
...
module.exports.loop = function(){
    ...
    for(var name in Game.spawns){
        ...
        var roads = Memstructures.run(Memory.rooms[MyRoom].RoomInfo.Roads);
        ...
        tower.run(MyRoom,towers,ramparts,containers,hostiles,roads);
    }
