...
module.exports.loop = function () {
    ...
    function NoCreeps(buil,upgr,work,harv,kill,AvailableEnergy,Hostiles){
        ...
        if(buil >= Nbuil && upgr >= Nupgr && work >= Nwork && harv >= Nharv && AvailableEnergy > 500) {
           Nharv = 4;
           Nwork = 4;
           Nupgr = 2;
           Nkill = 1;
    ...
    if(harvester.length < Nos[3]) {
        var newName = Game.spawns.Spawn1.createCreep(TransportCreep(0,8), undefined, {role: 'harvester'});
        console.log('Spawning new harvester: ' + newName);
    }
    if(defender.length < Nos[4] && harvester.length ==Nos[3] && worker.length == Nos[2] ) {
        var newName = Game.spawns.Spawn1.createCreep(ArmyCreep(Game.rooms[MyRoom].energyCapacityAvailable/2), undefined, {role: 'defender'});
